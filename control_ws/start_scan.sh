#!/bin/bash
# ======================================================
# EtherCAT 从站扫描节点启动脚本（SOEM ROS2 驱动）
...
NODE_PATH="$PROJECT_DIR/install/soem_ros2_driver/lib/soem_ros2_driver/slave_scan_node"

# ----------------------------------------
# 🧹 2. Ctrl+C 清理函数：退出后恢复网络
# ----------------------------------------
cleanup() {
  echo ""
  echo "---------------------------------------"
...
# ----------------------------------------
echo "---------------------------------------"
read -p "按 Enter 启动 EtherCAT 从站扫描节点..."

echo "🚀 启动 slave_scan_node ..."
